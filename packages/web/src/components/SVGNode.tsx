...
export const SVGNode: React.FC<{
  ...
  ySpacing: number;
  zoom: number;
}> = ({ node, parentX, parentY, startX, startY, dedicatedWidth, ySpacing, zoom }) => {
    ...
    <React.Fragment key={node.value.lp.name}>
      {parentX !== undefined && parentY !== undefined && (
        <g opacity={isVisited ? 1 : 0.5}>
          <line x1={zoom * parentX} y1={zoom * parentY} x2={zoom * x} y2={zoom * y} stroke="#444" strokeWidth={0.5} />
          <text x={zoom * (x + (parentX - x) / 2)} y={zoom * (y + (parentY - y) / 2)} textAnchor="middle" fontWeight={700} fontSize={zoom * 15}>
            {node.value.label}
          </text>
        </g>
      )}
      {node.left && <SVGNode parentX={x} parentY={y} startX={startX} startY={startY + ySpacing} dedicatedWidth={dedicatedWidth / 2} ySpacing={ySpacing} node={node.left} zoom={zoom} />}
      {node.right && (
